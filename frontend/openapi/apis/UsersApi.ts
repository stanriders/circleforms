...
export class UsersApi extends runtime.BaseAPI {
  ...
  async meAnswersGetRaw(
  ...
  ): Promise<runtime.ApiResponse<Array<AnswerPostContract>>> {
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) =>
      jsonValue.map(AnswerPostContractFromJSON)
    );
  }
   ...
   * Get user\'s answers (Auth)
   */
  async meAnswersGet(initOverrides?: RequestInit): Promise<Array<AnswerPostContract>> {
    const response = await this.meAnswersGetRaw(initOverrides);
    return await response.value();
  }

  /**
   * Get data for current user. (Auth)
   */
   ...
   * Get user data.
   */
  async usersIdMinimalGetRaw(
    requestParameters: UsersIdMinimalGetRequest,
    initOverrides?: RequestInit
  ): Promise<runtime.ApiResponse<UserMinimalContract>> {
    if (requestParameters.id === null || requestParameters.id === undefined) {
      throw new runtime.RequiredError(
        "id",
        "Required parameter requestParameters.id was null or undefined when calling usersIdMinimalGet."
      );
    }
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) =>
      UserMinimalContractFromJSON(jsonValue)
    );
  }
   ...
   * Get user data.
   */
  async usersIdMinimalGet(
    requestParameters: UsersIdMinimalGetRequest,
    initOverrides?: RequestInit
  ): Promise<UserMinimalContract> {
